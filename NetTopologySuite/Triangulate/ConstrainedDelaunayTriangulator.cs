...
namespace NetTopologySuite.Triangulate
{
    ...
    /// <summary>
    /// Computes a Constrained Delaunay Triangulation over a set of sites and a set of
    /// linear constraints.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public class ConstrainedDelaunayTriangulator
    {
        ...
        private IList<LineSegment> _breakLines = new List<LineSegment>();
        ...
        public ConstrainedDelaunayTriangulator(IEnumerable<Vertex> initialVertices,
                double tolerance)
        {
        ...
        /// <summary>
        /// Adds a constrained Segment into the Triangulation
        /// </summary>
        ...
        public bool AddSegment(Segment newSegment)
        {
            ...
            // Looking for first intersecting line on the subdivision
            QuadEdge.QuadEdge eCurrent = _subdiv.Locate(newSegment.Start);
            if (!eCurrent.Orig.Coordinate.Equals2D(newSegment.Start, 0.001) && !eCurrent.Dest.Coordinate.Equals2D(newSegment.Start, 0.001))
                eCurrent = _subdiv.Locate(newSegment.Start);
            if (eCurrent.Dest.Coordinate.Equals2D(newSegment.Start, 0.001))
                eCurrent = eCurrent.Sym;

            //Check if new Segment is already part of Triangulation
            QuadEdge.QuadEdge FirstEdge = eCurrent;
            do
            {
                // If it's already part of the Triangulation, problem is already solved
                if (eCurrent.Dest.Coordinate.Equals2D(newSegment.End, 0.001))
                {
                    return true;
                }
                eCurrent = eCurrent.ONext;
            } while (eCurrent != FirstEdge);

            do
            {
                eCurrent = eCurrent.OPrev;
            } while (newSegment.LineSegment.Intersection(eCurrent.RPrev.ToLineSegment()) == null);

            List<QuadEdge.QuadEdge> QeL = new List<QuadEdge.QuadEdge>();
            QeL.Add(eCurrent);
            eCurrent = eCurrent.RPrev;

            while(!newSegment.End.Equals2D(eCurrent.Dest.Coordinate, 0.001))
            {
                if (newSegment.LineSegment.Intersection(eCurrent.ToLineSegment()) != null)
                {
                    if (_breakLines.Contains(eCurrent.ToLineSegment()))
                        return false;
                    eCurrent = eCurrent.ONext;
                }
                else
            ...
            while(QeL.Count > 1)
            {
                // Looking for the first angle < PI
                int i = 0;
                while (!QeL[i+1].Dest.RightOf(QeL[i]))
                {
                    i++;
                }

                QuadEdge.QuadEdge eSwap = QeL[i].RPrev;

                while (QeL[i].RPrev != QeL[i + 1] || QeL[i + 1].OPrev != QeL[i].Sym)
                {
                    eSwap = QeL[i].RPrev;
                    while (!eSwap.LNext.Dest.RightOf(eSwap.RPrev))
                    {
                        eSwap = eSwap.ONext;
                    }
                    QuadEdge.QuadEdge.Swap(eSwap);
                }
            ...
            _breakLines.Add(new LineSegment(newSegment.Start, newSegment.End));
            return true;
        }
